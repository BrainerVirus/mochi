...
pub fn shell_background_from_theme(theme_name: Option<&str>) -> Color {
    let is_dark = theme_name.is_some_and(|name| name.to_lowercase().contains("dark"));
...
fn theme_background_color() -> Color {
    ...
    gtk::Settings::default()
        ...
        .map(|name| shell_background_from_theme(Some(name.as_str())))
        .unwrap_or_else(|| shell_background_from_theme(None))
}
