...
### Task 12: Preserve the existing settings wrapper

No implementation change is required. `SettingsTabSegmentedControl` remains in `tray-segmented-control.tsx`; moving it is unrelated to the chevron repair and would delete the live `TraySegmentedControlView` dependency. Verify the existing settings-form tests remain green.
