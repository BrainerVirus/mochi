...
- If a platform/window-manager combination still fails native controls, add a diagnostic mode that records decoration state and Tauri window events, then fall back to a custom titlebar with explicit minimize/maximize/close handlers only for affected Linux sessions.
...
- Add release validation that curls representative macOS and Linux endpoints for an older version and verifies HTTP 200 plus required updater fields.
...
- Add Rust tests for menu item labels and channel check state.
...
- Add workflow validation that curls representative old-version endpoints and checks JSON fields.
...
- Linux manual QA:
  ...
  - verify wheel scrolling and native controls.
- Release QA:
  - after publishing, curl representative update endpoints for `0.1.7` and `0.2.0`,
