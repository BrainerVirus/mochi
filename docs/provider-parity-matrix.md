...
## Mochi v1 providers (12)

| Provider    | CodexBar ID         | Usage windows (CodexBar labels)                                   | Strategies                                                       | Auth                                                                         | Status                     | Cost                              | Mochi       |
| ----------- | ------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------- | --------------------------------- | ----------- |
| Codex       | `codex`             | Session / Daily / Weekly (from window seconds)                    | OAuth API → CLI RPC → web dashboard (manual cookie)              | OAuth tokens, manual browser cookie, `codex` CLI                             | Statuspage.io (OpenAI)     | JSONL session scan (`CODEX_HOME`) | **done**    |
| Claude      | `claude`            | Session, Weekly (+ Sonnet/Opus weekly when present)               | OAuth API → Web API (manual cookie); Admin API + CLI PTY planned | OAuth (`~/.claude/.credentials.json`), `MOCHI_CLAUDE_*` env, browser session | Statuspage.io (Anthropic)  | JSONL project logs (planned)      | **partial** |
| Cursor      | `cursor`            | Total, Auto, API + on-demand USD (`providerCost`)                 | Web API (`MOCHI_CURSOR_COOKIE*`); browser import                 | Manual cookie header via env/file, Zen browser cookies                       | Statuspage.io (Cursor)     | On-demand spend meter             | **done**    |
| Gemini      | `gemini`            | Pro, Flash                                                        | OAuth-backed quota API (Gemini CLI credentials)                  | Google OAuth via Gemini CLI (`~/.gemini/oauth_creds.json`, `MOCHI_GEMINI_*`) | Google Workspace incidents | —                                 | **partial** |
| Copilot     | `copilot`           | Premium, Chat                                                     | OAuth token → `copilot_internal` API; device-flow UI planned     | `MOCHI_COPILOT_TOKEN*`, GitHub OAuth token(s)                                | Statuspage.io (GitHub)     | —                                 | **partial** |
| OpenCode    | `opencode`          | 5-hour, Weekly                                                    | Web `_server` dashboard                                          | Browser cookies (`opencode.ai`), `MOCHI_OPENCODE_COOKIE*`                    | —                          | —                                 | **partial** |
| OpenCode Go | `opencodego`        | 5-hour, Weekly, Monthly (optional) + Zen balance (`providerCost`) | Web `_server` + workspace page for Zen balance                   | Cookies + workspace ID (`MOCHI_OPENCODE_WORKSPACE_ID`)                       | —                          | Zen pay-as-you-go balance         | **partial** |
| Antigravity | `antigravity`       | —                                                                 | Local LSP/HTTP probe (`GetUserStatus`)                           | Local Antigravity language server                                            | Google Workspace incidents | —                                 | **stub**    |
| Factory     | `factory` / `droid` | —                                                                 | Web cookies → stored tokens → local storage → WorkOS cookies     | Factory/WorkOS session, bearer tokens                                        | status.factory.ai (link)   | —                                 | **stub**    |
| z.ai        | `zai`               | —                                                                 | API token → quota API                                            | `providers[].apiKey`, `Z_AI_API_KEY`                                         | none                       | —                                 | **stub**    |
| Kiro        | `kiro`              | —                                                                 | CLI: `kiro-cli chat --no-interactive "/usage"`                   | AWS Builder ID via `kiro-cli` login                                          | AWS Health (manual link)   | —                                 | **stub**    |
| Augment     | `augment`           | —                                                                 | CLI (`auggie`) → web cookies fallback                            | CLI session, browser cookies                                                 | none                       | —                                 | **stub**    |
...
| Cursor               | web cookies                 | manual cookie env/file          | Cursor Statuspage    | on-demand USD     | done    |
| OpenCode             | web dashboard               | browser cookies                 | —                    | —                 | partial |
| OpenCode Go          | web dashboard               | cookies + workspace ID          | —                    | Zen balance       | partial |
...
| Cookie import                             | Yes         | Cursor + OpenCode browser import     | 2+          |
| Usage cache / LKG                         | Yes         | Live fetch every call                | 1           |
| Provider metadata registry                | Yes         | `get_provider_catalog` + settings UI | 1 (partial) |
| Real fetch (6 stubs)                      | Yes         | Static snapshots                     | 3–4         |
| Codex full parity                         | Yes         | OAuth/CLI/cookies                    | 2           |
